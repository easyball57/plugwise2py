...
class PWControl(object):
    ...
    def process_mqtt_commands(self):
        ...
        while not qsub.empty():
            ...
            #Modification Jeedom
            #Topic format: plugwise/cmd/<mac>/<cmdname>
            st = topic.split('/')
            try:
                #Modification Jeedom
                #mac = st[-1]
    ...
    def ftopicJeedom(self, keyword, mac):
        return str("plugwise/state/" + mac +"/" + keyword)
    ...
    def publish_circle_state(self, mac):
        ...
        qpub.put((self.ftopicJeedom("circle", mac), str(self.get_status_json(mac)), True))
    ...
    def ten_seconds(self):
        ...
        for mac, f in self.actfiles.iteritems():
            ...
            try:
                ...
                #msg = str('%.2f' % (usage))
                qpub.put((self.ftopicJeedom("power", mac), msg, True))
            except ValueError:
                #print("%5d, " % (ts,))
                f.write("%5d, \n" % (ts,))
        ...
        if control['savelog'].lower() == 'yes':
            ...
            for dt, watt, watt_hour in log:
                if not dt is None:                
                    ...
                    qpub.put((self.ftopicJeedom("energy", mac), msg, True))
            if not f == None:
                f.close()
...
try:
    ...
    if  not mqtt:
        ...
        mqttclient.subscribe("plugwise/cmd/#")
